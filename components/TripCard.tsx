  ...
  return (
    <Link href={`/trips/${id}`} className="trip-card">
      ...
      <div className="trip-card-body">
        <h3>{name}</h3>
        <div className="dates">{fmtDate(startDate)} — {fmtDate(endDate)}</div>
        <div className="trip-stats">
          <div className="trip-stat">Noites<b>{nights}</b></div>
          <div className="trip-stat">Pessoas<b>{peopleCount}</b></div>
          <div className="trip-stat">Gasto<b>{fmtMoney(total)}</b></div>
        </div>
      </div>
    </Link>
  );
