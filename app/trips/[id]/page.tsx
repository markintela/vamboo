...
export default async function TripPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: trip, error } = await supabase
    .from('trips')
    .select('*, trip_people(*), trip_routes(*, places:trip_route_places(*)), trip_transports(*, documents:trip_transport_documents(*)), expenses(*), hotels(*, reservation_number_decrypted), trip_checklist_items(id, done)')
    .eq('id', id)
    .single();
